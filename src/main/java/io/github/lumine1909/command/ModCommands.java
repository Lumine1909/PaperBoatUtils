...
public class ModCommands {
    ...
    public static void registerCommands() {
        dispatcher.register(
            literal("stepsize").requires(ctx -> ctx.getBukkitSender().hasPermission("paperboatutils.admin")).then(
                argument("size", FloatArgumentType.floatArg()).executes(ctx -> {
                    ...
                    buf.writeFloat(size);
                    PacketHandler.syncPacket(new ClientboundCustomPayloadPacket(new DiscardedPayload(PaperBoatUtils.modKey, buf)));
                    return 1;
                })
            )
        );
        ...
        dispatcher.register(
            literal("collisionmode").requires(ctx -> ctx.getBukkitSender().hasPermission("paperboatutils.admin")).then(argument("ID", IntegerArgumentType.integer(0, 2)).executes(ctx -> {
                    ServerPlayer player = ctx.getSource().getPlayer();
                    if (player == null) return 0;
                    FriendlyByteBuf buf = new FriendlyByteBuf(Unpooled.buffer());
        ...
        dispatcher.register(
            literal("stepwhilefalling").requires(ctx -> ctx.getBukkitSender().hasPermission("paperboatutils.admin")).then(argument("enabled", BoolArgumentType.bool()).executes(ctx -> {
                ServerPlayer player = ctx.getSource().getPlayer();
                if (player == null) return 0;
                FriendlyByteBuf buf = new FriendlyByteBuf(Unpooled.buffer());
